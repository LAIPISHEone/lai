    ...
    <div class="main-content">
      ...
      <el-card class="calendar-card">
        <div slot="header" class="clearfix">
          <span>复习日历</span>
          <el-button style="float: right; padding: 3px 0" type="text" @click="generateSchedule">生成复习计划</el-button>
        </div>
...
export default {
  ...
  computed: {
    ...
    pendingReviews() {
      return this.reviewSchedule.filter(
        item => !item.completed && moment(item.date).isSameOrBefore(moment(), 'day')
      ).length;
    },
    ...
    upcomingReviews() {
      ...
      return this.reviewSchedule.filter(
        item => !item.completed && moment(item.date).isBetween(moment(), endDate, 'day', '[]')
      ).slice(0, 5); // 只显示最多5条
    },
    ...
    selectedDayEvents() {
      ...
      return this.reviewSchedule.filter(item => item.date === this.selectedDate);
    }
  },
  ...
  methods: {
    ...
    // 保存数据
    saveData() {
      localStorage.setItem('learningMaterials', JSON.stringify(this.learningMaterials));
    ...
    addMaterial() {
      const newMaterial = {
        ...
        firstStudyDate: moment(this.newMaterial.firstStudyDate).format('YYYY-MM-DD'),
        createdAt: moment().format('YYYY-MM-DD HH:mm:ss')
      };
      
      this.learningMaterials.push(newMaterial);
      this.newMaterial = { title: '', description: '', firstStudyDate: new Date() };
      this.saveData();
    ...
    generateSchedule() {
      ...
      this.learningMaterials.forEach(material => {
        this.selectedEbbinghausStages.forEach(days => {
          const reviewDate = moment(material.firstStudyDate).add(days, 'days').format('YYYY-MM-DD');
    ...
    completeReview(review) {
      const index = this.reviewSchedule.findIndex(item => item.id === review.id);
      if (index !== -1) {
...
.calendar-card, .stats-card, .content-card, .settings-card, .tasks-card {
  margin-bottom: 20px;
}
